...
class JsonStore:
    ...
    def read(self) -> dict[str, Any]:
        ...
        if "races" not in data or not isinstance(data["races"], list):
            data["races"] = []
        if "race_results" not in data or not isinstance(data["race_results"], list):
            data["race_results"] = []
        if "rankings" not in data or not isinstance(data["rankings"], list):
            data["rankings"] = []
        if "missions" not in data or not isinstance(data["missions"], list):
            data["missions"] = []
        if "vehicle_repairs" not in data or not isinstance(data["vehicle_repairs"], list):
            data["vehicle_repairs"] = []
        if "reputations" not in data or not isinstance(data["reputations"], list):
            data["reputations"] = []
        if "reputation_logs" not in data or not isinstance(data["reputation_logs"], list):
